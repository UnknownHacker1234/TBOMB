...
def do_zip_update():
    ...
    if success:
        ...
        mesgdcrt.WarningMessage("Grab The Latest one From https://github.com/TheMN/TBomb.git")
...
def do_git_update():
    ...
    if success:
        ...
        print("git checkout . && git pull https://github.com/TheMN/TBomb.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get("https://raw.githubusercontent.com/TheMN/TBomb/master/.version").text.strip()
    if fver != __VERSION__:
...
def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com/TheMN/TBomb/master/.notify").text.upper().strip()
        if len(noti) > 10:
